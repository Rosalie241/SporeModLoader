...
namespace Simulator
{
	...
	public:
		...
		/// @param orientation
		/// @returns The last bundle created
		cGameBundle* CreateGroundBundles(float amount, int bundleType, const Vector3& position, const Quaternion& orientation = Quaternion());


		// TODO: The 'Remove' functions have no disk addresses yet!
		...
		/// Removes a certain quantity from mpGroundContainer. This will remove as many bundles as necessary,
		/// and change the value of the last remaining bundle. If the amount is small enough, no bundle may be deleted.
		/// @param amount
		/// @param bundle If not null, removes only that specific bundle
		/// @returns The actual amount that has been removed.
		float RemoveGroundBundles(float amount, cGameBundle* bundle = nullptr);
