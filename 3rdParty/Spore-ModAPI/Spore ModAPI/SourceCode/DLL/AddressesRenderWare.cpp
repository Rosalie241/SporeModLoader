...
namespace RenderWare
{
	...
	{
		DefineAddress(SetTexture, SelectAddress(0x00FC2ED0, 0x00FC27A0));
	}

	namespace Addresses(CompiledState)
	{
		DefineAddress(Load, SelectAddress(0x011F0D00, 0x011EE5D0));
		DefineAddress(GetRaster, SelectAddress(0x011F0FF0, 0x11EE8C0));
		DefineAddress(SetRaster, SelectAddress(0x011F0E30, 0x011EE700));
	}

	namespace Addresses(Mesh)
	{
		DefineAddress(CalculateTriangleCount, SelectAddress(0x011FBD30, 0x011F95F0));
		DefineAddress(SetIndexBuffer, SelectAddress(0x011FBE50, 0x011F9710));
		DefineAddress(SetIndicesCount, SelectAddress(0x011FBE20, 0x011F96E0));
		DefineAddress(SetVertexBuffer, SelectAddress(0x011FBDE0, 0x011F96A0));
		DefineAddress(Render, SelectAddress(0x011FBE80, 0x011F9740));
	}
